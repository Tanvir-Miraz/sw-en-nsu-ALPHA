...
// SUPABASE_URL=https://ucrcjipzcrkmtilvewqr.supabase.co
// SUPABASE_ANON_KEY=sb_publishable_...
// Optional but recommended for backend uploads: SUPABASE_SERVICE_ROLE_KEY=sb_secret_...
...
if (!supabase) {
  console.warn('⚠️ Supabase is not configured. Report upload/view/share will not work until SUPABASE_URL and SUPABASE_ANON_KEY are set in .env.');
}
...
function getPublicReportUrl(storagePath) {
  if (!supabase || !storagePath) return null;
  const { data } = supabase.storage.from(SUPABASE_BUCKET).getPublicUrl(storagePath);
  return data.publicUrl;
}

console.log('✅ FINAL FIXED SERVER LOADED - report upload route accepts report/reportName aliases');
console.log('📁 Serving static files from:', frontendPath);
console.log('☁️ Supabase report bucket:', SUPABASE_BUCKET);

// Enhanced authentication middleware
function authenticateToken(req, res, next) {
  const userEmail = req.headers['x-user-email'] || req.headers['X-User-Email'] || req.body.userEmail || req.body.email || req.query.userEmail || req.query.email || 'demo@example.com';
    ...
    db.query(findUserQuery, [email], async (err, results) => {
      ...
      db.query(deleteOldTokensQuery, [user.user_id], (err) => {
        ...
        db.query(insertTokenQuery, [
          ...
          // Create reset URL
          const resetUrl = `http://localhost:5000/reset-password.html?token=${resetToken}&email=${encodeURIComponent(user.email)}`;
...
function normalizeReportRow(row) {
  return {
    ...
    download_url: row.download_url || getPublicReportUrl(row.storage_path),
    file_name: row.file_name,
    created_at: row.created_at
  };
}

function getReportNameFromRequest(req) {
  return String(
...
app.get('/api/reports', authenticateToken, async (req, res) => {
  try {
    ...
    db.query(query, [userId], (err, results) => {
      ...
      res.json({
        success: true,
        reports: (results || []).map(normalizeReportRow)
      });
    });
  } catch (error) {
...
// Upload report to Supabase Storage and save metadata in MySQL
// This handler is intentionally tolerant: it accepts report/reportFile/file/upload as file field,
// and report_name/reportName/title/name as report name field.
const reportUploadAny = multer({
  storage: multer.memoryStorage(),
...
function pickUploadedReportFile(req) {
  if (req.file) return req.file;
  if (!Array.isArray(req.files) || req.files.length === 0) return null;
  return (
    req.files.find(f => ['report', 'reportFile', 'file', 'upload', 'document'].includes(f.fieldname)) ||
    req.files[0]
  );
}
...
async function handleReportUpload(req, res) {
  try {
    if (!supabase) {
      return res.status(500).json({
        success: false,
        message: 'Supabase is not configured. Please set SUPABASE_URL and SUPABASE_ANON_KEY in .env'
      });
    }

    const userId = req.user.userId;
    const uploadedFile = pickUploadedReportFile(req);
    const reportName = getReportNameFromRequest(req);

    console.log('REPORT UPLOAD DEBUG:', {
      body: req.body,
      files: Array.isArray(req.files) ? req.files.map(f => ({ fieldname: f.fieldname, originalname: f.originalname, mimetype: f.mimetype, size: f.size })) : [],
      user: req.user
    });
    ...
    if (!uploadedFile) {
      return res.status(400).json({
        success: false,
        message: 'No report file uploaded. Use file field name report.'
      });
    }
    ...
    const safeOriginalName = makeSafeFileName(uploadedFile.originalname || 'report');
    const storagePath = `${userId}/${Date.now()}-${crypto.randomBytes(8).toString('hex')}-${safeOriginalName}`;
    ...
    if (uploadError) {
      ...
      return res.status(500).json({
        success: false,
        message: 'Failed to upload report to Supabase: ' + uploadError.message
      });
    }

    const downloadUrl = getPublicReportUrl(storagePath);
    const insertQuery = `
      INSERT INTO reports (user_id, report_name, storage_path, download_url, file_name)
      VALUES (?, ?, ?, ?, ?)
    `;

    db.query(insertQuery, [userId, reportName, storagePath, downloadUrl, uploadedFile.originalname], async (err, result) => {
      if (err) {
        await supabase.storage.from(SUPABASE_BUCKET).remove([storagePath]);
        console.error('Error inserting report metadata:', err);
        return res.status(500).json({
          success: false,
          message: 'Failed to save report metadata: ' + err.message
        });
      }
    ...
    console.error('Error in report upload:', error);
    return res.status(500).json({
      success: false,
      message: error.message || 'Internal server error during report upload'
    });
  }
}

app.post('/api/reports', authenticateToken, reportUploadAny, handleReportUpload);
app.post('/api/reports/upload', authenticateToken, reportUploadAny, handleReportUpload);
app.post('/api/upload-report', authenticateToken, reportUploadAny, handleReportUpload);

// Delete report from Supabase Storage and MySQL
app.delete('/api/reports/:reportId', authenticateToken, async (req, res) => {
  try {
    ...
    const selectQuery = 'SELECT storage_path FROM reports WHERE id = ? AND user_id = ?';
    db.query(selectQuery, [reportId, userId], async (err, results) => {
      if (err) {
      ...
      const storagePath = results[0].storage_path;
      const { error: removeError } = await supabase.storage
        .from(SUPABASE_BUCKET)
        .remove([storagePath]);

      if (removeError) {
        console.warn('Supabase delete warning:', removeError.message);
      }

      const deleteQuery = 'DELETE FROM reports WHERE id = ? AND user_id = ?';
      db.query(deleteQuery, [reportId, userId], (deleteErr) => {
        if (deleteErr) {
          console.error('Error deleting report metadata:', deleteErr);
          return res.status(500).json({
            success: false,
            message: 'Failed to delete report metadata'
          });
        }
...
app.listen(PORT, () => {
  ...
  console.log(`   http://localhost:${PORT}/api/reports - Reports list/upload`);
  console.log(`   http://localhost:${PORT}/api/reports/:reportId - Delete report`);
